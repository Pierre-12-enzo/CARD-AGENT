...
const createTransporter = () => {
    ...
    if (process.env.NODE_ENV === 'production') {
        // Use Brevo (Sendinblue) - FREE & RELIABLE
        if (!process.env.BREVO_SMTP_KEY) {
            console.error('❌ CRITICAL: BREVO_SMTP_KEY not set in environment variables');
            console.error('📧 Please add BREVO_SMTP_KEY to your Render environment variables');
        }
        ...
        return nodemailer.createTransport({
            ...
            secure: false,
            auth: {
                user: process.env.BREVO_SMTP_USER || 'your@email.com',
...
const verifyConnection = async () => {
    try {
        ...
        console.log(`📧 Using SMTP: ${transporter.options.host}:${transporter.options.port}`);

        if (process.env.NODE_ENV === 'production') {
            console.log('🚀 Production email service active with Brevo');
        }
    } catch (error) {
        ...
        if (process.env.NODE_ENV === 'production') {
            ...
            console.error('📧 Check your Brevo credentials in Render environment variables');
        } else {

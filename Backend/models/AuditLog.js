...
// ===== COMPOUND INDEXES =====
auditLogSchema.index({ companyId: 1, createdAt: -1 }); // Company's history
auditLogSchema.index({ companyId: 1, action: 1, createdAt: -1 }); // Company + action
...
auditLogSchema.pre('save', async function (next) {
    ...
    //Auto-populate Company info if not provided
    if (this.companyId && !this.companyInfo?.name) {
        try {
            const Company = mongoose.model('Company');
            const company = await Company.findById(this.companyId).select('name');
            if (company) {
                this.companyInfo = {
                    name: company.name
                };
            ...
            console.error('Error populating company info in audit log:', error);
        }
    }

// routes/company.js - CARD-AGENT (FIXED ROUTE ORDER)
...
const { uploadCompanyLogo, deleteImage } = require('../utilis/cloudinaryAuth');
...
/**
 * @desc    Check company name availability
 * @route   GET /api/company/check-name/:name
 */
...
router.get('/',
    ...
    async (req, res) => {
        try {
            ...
            const query = {};
            if (status === 'active') query['license.status'] = 'active';
            if (status === 'pending') query['license.status'] = 'pending';
            if (status === 'revoked') query['license.status'] = 'revoked';
            if (search) {
                query.$or = [
                    { name: { $regex: search, $options: 'i' } },
...
router.get('/:id',
    ...
    async (req, res) => {
        try {
            ...
            const organizations = await School.find({ companyId: company._id }).select('name type stats');

            res.json({
                success: true,
                company: {
                    ...company.toObject(),
                    stats: { organizations: orgCount, students: studentCount, coWorkers: staffCount },
                    organizations
                }
            });
...
router.put('/profile',
    ...
    async (req, res) => {
        try {
            ...
            if (req.file) {
                try {
                    if (company.logo?.publicId) await deleteImage(company.logo.publicId);
                    const base64Image = `data:${req.file.mimetype};base64,${req.file.buffer.toString('base64')}`;
                    const logoData = await uploadCompanyLogo(base64Image, `company_${company._id}`);
                    company.logo = { url: logoData.url, publicId: logoData.publicId };
                } catch (uploadError) {
            ...
            res.json({
                ...
                company: { id: company._id, name: company.name, email: company.email, phone: company.phone, logo: company.logo?.url }
            });
...
/**
 * @desc    Generate/Activate license (super admin)
...
router.post('/:id/license',
    ...
    async (req, res) => {
        try {
            ...
            res.json({
                success: true,
                message: emailSent ? 'License activated! Key has been emailed to the admin.' : 'License activated!',
                license: {
                    key: licenseKey,
                    status: 'active',
                    maxOrganizations,
...
/**
 * @desc    Revoke license (super admin)
...
router.post('/:id/revoke-license',
    ...
    async (req, res) => {
        try {
            ...
            // 🔥 EMIT SOCKET EVENT TO KICK OUT THE ADMIN
            const socketService = require('../services/socketService');
            if (adminUser) {
                socketService.emitToUser(adminUser._id.toString(), 'license:revoked', {
                    message: 'Your license has been revoked. Please contact support.',
                    companyName: company.name
                });

                // Also emit to all connections from this company
                socketService.emitToCompany(company._id.toString(), 'license:revoked', {
                    message: 'Company license has been revoked.',
                    companyName: company.name
                });
            }
            ...
            try {
                if (adminUser) {
                    const { sendLicenseRevokedEmail } = require('../utilis/emailService');
                    await sendLicenseRevokedEmail(adminUser, company, req.body.reason || 'License revoked by administrator');
                }
...
/**
 * @desc    Update license limits (super admin)

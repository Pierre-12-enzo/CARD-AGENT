// pages/co-worker/Overview.jsx - CARD-AGENT CO-WORKER DASHBOARD
...
const CoWorkerOverview = () => {
  ...
  const fetchOverview = async () => {
    try {
      ...
      const orgStats = await Promise.all(perms.map(async (perm) => {
        try {
          const res = await studentAPI.getByOrganization(perm.organizationId, { limit: 1 });
          const totalPeople = res?.pagination?.total || 0;
          return { ...perm, totalPeople };
        } catch (e) {
          return { ...perm, totalPeople: 0 };
        }
      }));

      const totalStudents = orgStats.reduce((sum, o) => sum + o.totalPeople, 0);
      ...
      if (orgStats.length > 0) {
        activity.push({ action: `${orgStats.length} organization(s) assigned`, time: 'Active', icon: 'pi-building', color: 'red' });
      }
      activity.push({ action: `${totalStudents} total records`, time: 'Current', icon: 'pi-users', color: 'slate' });
      activity.push({ action: 'Dashboard session started', time: 'Just now', icon: 'pi-desktop', color: 'slate' });
      activity.push({ action: 'Logged into your account', time: 'Just now', icon: 'pi-sign-in', color: 'red' });

      setStats({
        assignedOrgs: orgStats,
        totalStudents,
        totalCards,
  ...
  if (activePermissions.includes('canManageStudents')) quickActions.push({ label: 'Students', icon: 'pi-users', path: '/co-worker/students', color: 'slate' });
  if (activePermissions.includes('canManageTemplates')) quickActions.push({ label: 'Templates', icon: 'pi-image', path: '/co-worker/templates', color: 'slate' });
  ...
  if (activePermissions.includes('canUploadPhotos')) quickActions.push({ label: 'Photos', icon: 'pi-camera', path: '/co-worker/photos', color: 'green' });
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard title="Your Role" value="CO-WORKER" icon="pi-user" color="red" subtitle={`${userPermissions.length} permission types`} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-slate-200/50 overflow-hidden">
            <div className="bg-gradient-to-r from-red-600 to-red-500 px-5 py-4">
              ...
              <p className="text-red-100 text-sm mt-1">What you can do</p>
            </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {stats.assignedOrgs.length > 0 && (
            <div className="bg-white rounded-2xl shadow-lg border border-slate-200/50 p-5">
              <h3 className="font-semibold text-slate-800 mb-4 flex items-center">
                <i className="pi pi-building mr-2 text-red-600"></i>Your Organizations
              </h3>
                ...
                {stats.assignedOrgs.map((org, i) => (
                  <div key={i} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="font-medium text-slate-800 text-sm">{org.organizationName}</p>
                        <p className="text-xs text-slate-500">{org.totalPeople} people</p>
                      </div>
                    </div>
                    <div className="flex gap-1">
                      {org.canGenerateCards && <span className="text-xs bg-red-50 text-red-600 px-2 py-0.5 rounded-full">Cards</span>}
                      {org.canManageStudents && <span className="text-xs bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full">Students</span>}
                    </div>
                  </div>
                ))}
              </div>
